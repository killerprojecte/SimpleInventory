...
public class SimpleInventory {
    ...
    public static class InventoryParser {
        public static SimpleInventory deserialize(FileConfiguration configuration, Map<String, ClickHandlers> clickHandlers, CloseHandler closeHandler, Map<String, CloseSlotHandler> closeSlotHandlers) {
            return deserialize(configuration, clickHandlers, closeHandler, closeSlotHandlers, string -> string);
        }
        public static SimpleInventory deserialize(FileConfiguration configuration, Map<String, ClickHandlers> clickHandlers, CloseHandler closeHandler, Map<String, CloseSlotHandler> closeSlotHandlers, Function<String, String> variableHandler) {
            List<String> layouts = configuration.getStringList("layout");
            SimpleInventory inventory = new SimpleInventory(variableHandler.apply(configuration.getString("title")), layouts.size() * 9);
            ...
            for (String key : configuration.getConfigurationSection("icons").getKeys(false)) {
                ...
                if (section.contains("lore")) {
                    itemBuilder.lore(section.getStringList("lore").stream().map(variableHandler).collect(Collectors.toList()));
                }
